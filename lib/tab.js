...
function onFirstTabCreated(terminal) {
    try {
        ...
        const { actualCellWidth, actualCellHeight } = terminal._core._renderCoordinator.dimensions;
        const elementStyle = window.getComputedStyle(terminal.element.querySelector('.xterm-screen'));
        electron_1.remote.getCurrentWindow().setSize(window.outerWidth - parseInt(elementStyle.getPropertyValue('width'), 10) + actualCellWidth * cols, window.outerHeight - parseInt(elementStyle.getPropertyValue('height'), 10) + actualCellHeight * rows);
    }
    catch (e) {
        console.error(e);
    }

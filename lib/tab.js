...
const pty_options_1 = require("./pty-options");
let Tab = /** @class */ (() => {
    class Tab {
        constructor(tabContainer, contentContainer, pause) {
            ...
            this.disposables = [];
            this.active = true;
            tabContainer.appendChild(this.tabElement = hyperscript_1.default("a", { className: "item", onclick: Tab.handleTabClick, onauxclick: Tab.handleTabMouseUp, oncontextmenu: e => {
                    e.preventDefault();
                    pty_options_1.togglePriority();
                }, draggable: true },
        ...
        async attach(pty) {
            ...
            }), attachDisposable(pty, 'data', this.handleDataOutput), attachDisposable(pty, 'error', err => this.printDisposableMessage(`Oops... error: ${err.message || err}`)), attachDisposable(pty, 'end', this.pause ?
                ((code, signal) => this.printDisposableMessage(`Program exits with ${code} ${code_to_signal_1.default(signal) || ''}`, false)) :
                this.dispose));

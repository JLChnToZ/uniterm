...
class Tab {
    constructor(tabContainer, contentContainer, pause) {
        ...
        this.disposables = [];
        this.active = true;
        tabContainer.appendChild(this.tabElement = hyperscript_1.default("a", { className: "item", onclick: Tab.handleTabClick, onmouseup: Tab.handleTabMouseUp, draggable: true },
    ...
    async attach(pty) {
        ...
        this.disposables.push(this.terminal.onData(this.handleDataInput), this.terminal.onResize(({ cols, rows }) => this.pty.resize(cols, rows)), this.terminal.onTitleChange(title => {
            ...
            ((code, signal) => this.printDisposableMessage(`Program exits with ${code} ${codeToSignal(signal) || ''}`, false)) :
            this.dispose));
...
Tab.tabs = new Map();
tslib_1.__decorate([
    decorators_1.readonly, decorators_1.bind

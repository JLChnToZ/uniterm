...
if (argv.config || argv['reset-config'])
    (async (resetConfig) => {
        try {
            ...
            if (argv.config)
                electron_1.shell.openExternal(config_1.configFilePath);
        }
        catch (e) {
    ...
    }).on('show-config', () => electron_1.shell.openExternal(config_1.configFilePath)).on('create-terminal-request', async (_, options) => (await getWindow(true)).send('create-terminal', options));
}

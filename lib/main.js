...
if (argv.pipe) {
    ...
    }).on('show-config', () => electron_1.shell.openItem(config_1.configFilePath)).on('create-terminal-request', async (e, options) => (await getWindow(true)).send('create-terminal', options));
}
...
function createWindow() {
    const window = new electron_1.BrowserWindow({
        ...
        frame: false,
        transparent: config_1.configFile && config_1.configFile.misc && config_1.configFile.misc.vibrancy,

...
const wheelObservable = rxjs_1.fromEvent(body, 'wheel')
    .pipe(operators_1.filter(e => e.ctrlKey && e.target.matches('.pty-container *')));
wheelObservable.subscribe(domutils_1.interceptEvent);
rxjs_1.merge(wheelObservable.pipe(operators_1.map(e => e.deltaZ || e.deltaY)), rxjs_1.fromEventPattern(cb => zoom_gesture_detector_1.startDetect(body, cb, true), (_, disposable) => disposable.dispose()).pipe(operators_1.scan((a, b) => a - b, 0), operators_1.debounceTime(250))).subscribe(delta => {

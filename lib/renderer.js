...
const header = h("div", { className: "header pty-tabs" },
    tabContainer,
    h("a", { className: "icon item", onclick: async () => {
            await config_1.loadConfig();
            new tab_1.Tab().attach(selector_1.createBackend({
...
const layoutContainer = document.body.appendChild(h("div", { className: "layout-container" }, header));
tab_1.setContainers(tabContainer, layoutContainer);
...
electron_1.ipcRenderer.on('create-terminal', async (e, options) => {
    await config_1.loadConfig();
    const tab = new tab_1.Tab(options.pause);

...
config_1.events.on('config', () => {
    ...
    if (config_1.configFile.mods && config_1.configFile.mods.length) {
        const userData = remote_wrapper_1.electron.app.getPath('userData');
...
class Tab {
    constructor(pause) {
        ...
        this.tabContent = h("div", { ondragenter: this.handleDragOver.bind(this), ondragover: this.handleDragOver.bind(this), ondrop: this.handleDrop.bind(this), oncontextmenu: this.handleContextMenu.bind(this) });
    ...
    async attach(pty) {
        ...
        }), attachDisposable(pty, 'data', this.handleDataOutput.bind(this)), attachDisposable(pty, 'end', this.pause ?

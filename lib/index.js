...
const WSLExecShell = require('./shells/wslexec');
const MinShell = require('./shells/minsh');
...
function createWindow() {
  try {
    ...
    const cwd = commander.cwd;
    const env = Object.assign({}, process.env, commander.env);
    return new TerminalHost(args.length ? args[0] === 'wsl' && process.platform === 'win32' ?
      new WSLExecShell(args.slice(1), cwd, env) :
      new ExecShell(args, cwd, env) :
      new MinShell(cwd, env));
  } catch(e) {
